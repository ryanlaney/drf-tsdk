...
class InnerBarSerializer(serializers.Serializer):
    ...
    b3 = InnerInnerBarSerializer(allow_null=True, required=False)


@ts_api_interface(name="IBar")
class BarSerializer(serializers.Serializer):
    a1 = serializers.CharField()
    a2 = serializers.DecimalField(max_digits=7, decimal_places=6, allow_null=True)
...
@ts_api_endpoint(
    ...
    response_serializer=BarSerializer(many=True),
)
@api_view(["GET"])
def bar(request):
    pass
